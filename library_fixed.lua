...
function Library.new(options)
        ...
        for n, page in pairs(state.pages) do
            ...
            if page.navButton then
                if n == name then
                    ...
                    if page.navTitleLabel then
                        TweenService:Create(page.navTitleLabel, TW_FAST, { TextColor3 = THEME.TEXT_PRIMARY }):Play()
                    end
                else
                    TweenService:Create(page.navButton, TW_FAST, { BackgroundColor3 = THEME.BG_BUTTON }):Play()
                    if page.navTitleLabel then
                        TweenService:Create(page.navTitleLabel, TW_FAST, { TextColor3 = THEME.TEXT_MUTED }):Play()
                    end
                end
            end
        end
    ...
    local function toggleSidebar()
        ...
        for _, page in pairs(state.pages) do
            if page.navButton then
                if page.navTitleLabel then
                    page.navTitleLabel.Visible = state.sidebarExpanded
                end
            end
        end
