...
local SIDEBAR_EXPANDED  = 216
local SIDEBAR_COLLAPSED = 56
local NEW_TOPBAR_H      = 0
local TOPBAR_H          = 52
...
function UserSettingsSection:Dropdown(configOrTitle, values, default, callback)
	...
	local arrow = Create("ImageLabel", {
		...
		-- Keep the chevron anchored to the fixed header row.
		-- Do not use Y scale here, because SettingsDropdown grows in height
		-- when opened; a 0.5 Y position makes the icon get dragged down.
...
function Window:_SetActivePage(page)
	...
	for _, p in ipairs(self.Pages) do
		...
		if p.Nav.Chevron then
			TweenService:Create(p.Nav.Chevron, TW_FAST, { ImageTransparency = (selected and not self.SidebarClosed) and 0.15 or 1 }):Play()
		end
		TweenService:Create(p.Nav.Button, TW_FAST, {
			-- Selected state follows the sidebar reference: subtle dark pill + accent bar.
			BackgroundColor3 = selected and THEME.BG_HOVER or THEME.BG_SIDEBAR,
...
function Window:_ReflowNav(animate)
	...
	local y = isClosed and (self.SidebarNavStartYClosed or 192) or (self.SidebarNavStartY or 150)
	...
	for _, item in ipairs(self.SidebarItems or {}) do
		if item.Type == "Page" and item.Page and item.Page.Nav then
			...
			apply(page.Nav.Button, {
				Position = isClosed and UDim2.new(0.5, -18, 0, y) or UDim2.new(0, 26, 0, y),
				Size = isClosed and UDim2.new(0, 36, 0, 36) or UDim2.new(1, -52, 0, 36),
			})

			-- Keep nav icons fixed-size when collapsed. Image icons no longer
			-- stretch to fill the whole selected tab square.
			apply(page.Nav.Icon, {
				Size = isClosed and UDim2.new(0, 20, 0, 20) or UDim2.new(0, 18, 0, 18),
				Position = isClosed and UDim2.new(0.5, -10, 0.5, -10) or UDim2.new(0, 21, 0.5, -9),
			})

			apply(page.Nav.Text, {
				TextTransparency = isClosed and 1 or 0,
			})
			if page.Nav.Chevron then
				apply(page.Nav.Chevron, { ImageTransparency = (isClosed or self.CurrentPage ~= page) and 1 or 0.15 })
			end
		...
		elseif item.Type == "Divider" and item.Frame then
			...
			apply(item.Frame, {
				Position = isClosed and UDim2.new(0.5, -13, 0, y + 7) or UDim2.new(0, 26, 0, y + 7),
				Size = isClosed and UDim2.new(0, 26, 0, 1) or UDim2.new(1, -52, 0, 1),
...
function Window:SetSidebarExpanded(expanded)
	...
	if toggleIcon then
		local nextImage = ResolveIcon(isExpanded and "solar:alt-arrow-left-linear" or "solar:alt-arrow-right-linear")
	...
	if self.AppSubtitleLabel then TweenService:Create(self.AppSubtitleLabel, TW_SIDEBAR, { TextTransparency = isExpanded and 0.05 or 1 }):Play() end
	...
	TweenService:Create(self.SidebarSearch, TW_SIDEBAR, {
		...
		Position = isExpanded and UDim2.new(0, 26, 0, 100) or UDim2.new(0.5, -18, 0, 142),
...
function Window:Page(name, icon)
	...
	local navY = (self.SidebarNavStartY or 150) + (#self.Pages * 41)
	local navButton = Create("TextButton", {
		Name = "NavItem_" .. pageName,
		Size = UDim2.new(1, -52, 0, 36),
		Position = UDim2.new(0, 26, 0, navY),
		BackgroundColor3 = THEME.BG_SIDEBAR,
		BackgroundTransparency = 1,
		Text = "",
		AutoButtonColor = false,
	...
	if iconIsImage then
		iconText = Create("ImageLabel", {
			Name = "Icon",
			Size = self.SidebarClosed and UDim2.new(0, 20, 0, 20) or UDim2.new(0, 18, 0, 18),
			Position = self.SidebarClosed and UDim2.new(0.5, -10, 0.5, -10) or UDim2.new(0, 21, 0.5, -9),
			BackgroundTransparency = 1,
			Image = pageIcon,
	...
	local selectedChevron = Create("ImageLabel", {
		Name = "SelectedChevron",
		Size = UDim2.new(0, 12, 0, 12),
		Position = UDim2.new(1, -24, 0.5, -6),
		BackgroundTransparency = 1,
		Image = ResolveIcon("solar:alt-arrow-right-linear"),
	...
	navButton.MouseEnter:Connect(function()
		...
		if self.SidebarClosed then
			self:_ShowSidebarTooltip(pageName, navButton.AbsolutePosition.Y - self.Window.AbsolutePosition.Y)
...
function Window:_ShowSidebarTooltip(text, y)
	if not self.SidebarClosed or not self.SidebarTooltip then return end
	local tooltipText = self.SidebarTooltip:FindFirstChild("Text")
	if tooltipText then tooltipText.Text = tostring(text or "") end
	self.SidebarTooltip.Position = UDim2.new(0, (self.Sidebar and self.Sidebar.AbsoluteSize.X or SIDEBAR_COLLAPSED) + 8, 0, math.max(8, (y or 0) + 4))
	self.SidebarTooltip.Visible = true
	TweenService:Create(self.SidebarTooltip, TW_FAST, { BackgroundTransparency = 0.04 }):Play()
	if self.SidebarTooltipStroke then TweenService:Create(self.SidebarTooltipStroke, TW_FAST, { Transparency = 0.25 }):Play() end
	local tooltipText = self.SidebarTooltip:FindFirstChild("Text")
	if tooltipText then TweenService:Create(tooltipText, TW_FAST, { TextTransparency = 0 }):Play() end
end

function Window:_HideSidebarTooltip()
	if not self.SidebarTooltip then return end
	TweenService:Create(self.SidebarTooltip, TW_FAST, { BackgroundTransparency = 1 }):Play()
	if self.SidebarTooltipStroke then TweenService:Create(self.SidebarTooltipStroke, TW_FAST, { Transparency = 1 }):Play() end
	local tooltipText = self.SidebarTooltip:FindFirstChild("Text")
	if tooltipText then TweenService:Create(tooltipText, TW_FAST, { TextTransparency = 1 }):Play() end
	task.delay(0.16, function()
		if self.SidebarTooltip and self.SidebarTooltip.BackgroundTransparency >= 0.95 then
			self.SidebarTooltip.Visible = false
		end
	end)
end
...
function Library.new(config)
	...
	local sidebarSubtitle = tostring(config.SidebarSubtitle or config.SideBarSubtitle or config.Subtitle or "eovo.team")
	...
	lineBelowNewTopBar.Visible = false
	...
	local sidebarToggleIcon = Create("ImageLabel", {
		...
		BackgroundTransparency = 1,
		Image = ResolveIcon("solar:alt-arrow-left-linear"),
	...
	local dotHolder = Create("Frame", { Name = "DotHolder", Size = UDim2.new(0, 52, 0, 18), Position = UDim2.new(0, 30, 0, 18), BackgroundTransparency = 1, ZIndex = 12, Parent = newTopBar })
	...
	dotHolder.Parent = sidebarTop
	dotHolder.Position = UDim2.new(0, 30, 0, 18)
	dotHolder.Size = UDim2.new(0, 52, 0, 18)
	sidebarToggle.Parent = sidebarTop
	sidebarToggle.Position = UDim2.new(1, -50, 0, 44)
	sidebarToggle.Size = UDim2.new(0, 36, 0, 36)
	local logoBox = Create("Frame", {
		Name = "LogoBox",
		Size = UDim2.new(0, sidebarLogoSize, 0, sidebarLogoSize),
		Position = UDim2.new(0, 30, 0, 44),
	...
	local appNameOffset = 30 + sidebarLogoSize + (sidebarLogoBgEnabled and 12 or 9)
	local appNameLabel = Create("TextLabel", { Name = "AppName", Size = UDim2.new(1, -(appNameOffset + 46), 0, 18), Position = UDim2.new(0, appNameOffset, 0, 45), BackgroundTransparency = 1, Text = title, Font = FONT_BOLD, TextSize = 14, TextColor3 = THEME.TEXT_ACCENT, TextXAlignment = Enum.TextXAlignment.Left, TextTruncate = Enum.TextTruncate.AtEnd, ZIndex = 8, Parent = sidebarTop })
	local appSubtitleLabel = Create("TextLabel", { Name = "AppSubtitle", Size = UDim2.new(1, -(appNameOffset + 46), 0, 14), Position = UDim2.new(0, appNameOffset, 0, 64), BackgroundTransparency = 1, Text = sidebarSubtitle, Font = FONT_REG, TextSize = 10, TextColor3 = THEME.TEXT_MUTED, TextTransparency = 0.05, TextXAlignment = Enum.TextXAlignment.Left, TextTruncate = Enum.TextTruncate.AtEnd, ZIndex = 8, Parent = sidebarTop })

	local sidebarSearch = Create("Frame", { Name = "SidebarSearch", Size = UDim2.new(1, -52, 0, 36), Position = UDim2.new(0, 26, 0, 100), BackgroundColor3 = THEME.BG_SEARCH, BorderSizePixel = 0, ZIndex = 7, Parent = sidebar })
	...
	local sidebarTooltip = Create("Frame", { Name = "SidebarTooltip", Size = UDim2.new(0, 0, 0, 28), AutomaticSize = Enum.AutomaticSize.X, BackgroundColor3 = THEME.BG_BUTTON, BackgroundTransparency = 1, BorderSizePixel = 0, Visible = false, ClipsDescendants = true, ZIndex = 150, Parent = root })
	Corner(8, sidebarTooltip)
	local sidebarTooltipStroke = Stroke(sidebarTooltip, THEME.BORDER, 1)
	sidebarTooltipStroke.Transparency = 1
	Padding(sidebarTooltip, 0, 10, 0, 10)
	local sidebarTooltipText = Create("TextLabel", { Name = "Text", Size = UDim2.new(0, 0, 1, 0), AutomaticSize = Enum.AutomaticSize.X, BackgroundTransparency = 1, Text = "", Font = FONT_SEMI, TextSize = 11, TextColor3 = THEME.TEXT_SECONDARY, TextTransparency = 1, TextXAlignment = Enum.TextXAlignment.Center, TextYAlignment = Enum.TextYAlignment.Center, ZIndex = 151, Parent = sidebarTooltip })
	...
	local self = setmetatable({
		...
		SidebarTooltipStroke = sidebarTooltipStroke,
